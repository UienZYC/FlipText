...
class SettingsApp:
    ...
    def _build_models_tab(self, parent: ttk.Frame) -> None:
        ...
        self.provider_box = ttk.LabelFrame(details, text="Provider", padding=10)
        self.provider_box.grid(row=0, column=0, sticky="ew")
        self.provider_box.columnconfigure(1, weight=1)
        ttk.Label(self.provider_box, text="Provider name").grid(row=0, column=0, sticky="w")
        ttk.Entry(self.provider_box, textvariable=self.provider_name_var).grid(row=0, column=1, sticky="ew", pady=(0, 6))
        ttk.Label(self.provider_box, text="Base URL").grid(row=1, column=0, sticky="w")
        ttk.Entry(self.provider_box, textvariable=self.base_url_var).grid(row=1, column=1, sticky="ew", pady=(0, 6))
        ttk.Label(self.provider_box, text="API key").grid(row=2, column=0, sticky="w")
        key_entry = ttk.Entry(self.provider_box, textvariable=self.api_key_var, show="*")
        key_entry.grid(row=2, column=1, sticky="ew")
        ttk.Button(self.provider_box, text="Show/Hide", command=lambda: self._toggle_key_visibility(key_entry)).grid(row=2, column=2, padx=(8, 0))

        self.model_box = ttk.LabelFrame(details, text="Model", padding=10)
        self.model_box.grid(row=1, column=0, sticky="nsew", pady=(10, 0))
        self.model_box.columnconfigure(1, weight=1)
        ttk.Label(self.model_box, text="Model name").grid(row=0, column=0, sticky="w")
        ttk.Entry(self.model_box, textvariable=self.model_name_var).grid(row=0, column=1, sticky="ew", pady=(0, 6))
        ttk.Label(self.model_box, text="Timeout (ms)").grid(row=1, column=0, sticky="w")
        ttk.Entry(self.model_box, textvariable=self.timeout_var).grid(row=1, column=1, sticky="ew", pady=(0, 6))
        ttk.Checkbutton(self.model_box, text="Enabled", variable=self.model_enabled_var).grid(row=2, column=1, sticky="w", pady=(0, 6))
    ...
    def _build_prompts_tab(self, parent: ttk.Frame) -> None:
        ...
        ttk.Label(
            ...
            text=(
                "Available variables:\n"
                "{text}: the selected source text\n"
                "{source_lang}: detected source language code\n"
                "{target_lang}: detected target language code\n"
                "Rule: if text contains Chinese characters, source_lang=zh and target_lang=en; otherwise source_lang=en and target_lang=zh."
            ),
            justify="left",
